...
from src.schemas.silver_schemas import SocialsSchema
...
GOLD_TOPIC = os.getenv("REDPANDA_GOLD_SOCIALS_TOPIC", "gold.socials.sentiment")
...
def shutdown_handler(signum, frame):
    print("[GOLD:SOCIALS] Shutdown signal received", flush=True)
    os._exit(0)
...
def validate_env():
    missing = []
    if not SILVER_TOPIC:
        missing.append("REDPANDA_SILVER_SOCIALS_TOPIC")
    ...
    if missing:
        print(f"[GOLD:SOCIALS] ERROR: Missing env vars: {missing}", flush=True)
        sys.exit(1)
...
def wait_for_broker():
    ...
    while True:
        attempt += 1
        if validate_broker():
            print("[GOLD:SOCIALS] Broker connected", flush=True)
            return
        wait_time = min(30, 5 * attempt)
        print(f"[GOLD:SOCIALS] Waiting for broker... (attempt {attempt}, next in {wait_time}s)", flush=True)
...
def create_pipeline():
    ...
    print(f"[GOLD:SOCIALS] Output: {GOLD_TOPIC}", flush=True)
    print(f"[GOLD:SOCIALS] Consumer: {consumer['group.id']}", flush=True)
    ...
    final = enriched.groupby(pw.this.symbol).reduce(
        ...
        socials_sentiment_scores=pw.reducers.tuple(pw.this.sentiment),
    )

    if DEBUG:
        os.makedirs("debug_output/gold", exist_ok=True)
        pw.io.jsonlines.write(final, "debug_output/gold/socials_sentiment.jsonl")
    else:
        pw.io.kafka.write(
            final,
    ...
    print("[GOLD:SOCIALS] Pipeline ready", flush=True)
...
def run_pipeline() -> str:
    try:
        ...
        if any(kw in error_str for kw in ["resolve", "connection", "timeout", "kafka", "broker", "network", "host"]):
            print(f"[GOLD:SOCIALS] Connection error: {e}", flush=True)
            return "connection"
        print(f"[GOLD:SOCIALS] Runtime error: {e}", flush=True)
        return "error"


def main():
    print("[GOLD:SOCIALS] Starting...", flush=True)
    ...
    try:
        ...
        print(f"[GOLD:SOCIALS] License error: {e}", flush=True)
        sys.exit(1)
    ...
    while True:
        ...
        try:
            ...
            print(f"[GOLD:SOCIALS] Pipeline creation failed: {e}", flush=True)
            time.sleep(10)
            continue
        ...
        if result == "connection":
            print("[GOLD:SOCIALS] Connection lost. Reconnecting...", flush=True)
            time.sleep(5)
            continue
        
        if result == "error":
            print("[GOLD:SOCIALS] Restarting in 10s...", flush=True)
            time.sleep(10)
            continue
    ...
    print("[GOLD:SOCIALS] Shutdown complete", flush=True)
